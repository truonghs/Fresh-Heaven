...
module.exports = {
  ...
  forgotPassword: async (req, res) => {
    try {
      ...
      res.status(500).json({
        message:
          "An error occurs while sending the email. Please try again later!",
      });
    }
  },
  ...
  resetPassword: async (req, res) => {
    try {
      ...
      if (
        user.passwordToken.passwordToken == token ||
        user.passwordToken.expireAt >= new Date()
      ) {
        console.log(token);
        ...
        res
          .status(405)
          .json({ message: "Reset password token incorrect or expired!" });
      }
    } catch (error) {
      res.status(500).json({
        message:
          "An error occurs while changing password. Please try again later!",
      });
    }
  },
};
